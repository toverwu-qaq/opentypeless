...
| 클라우드 의존성 | 없음 — 모든 요청이 제공자에게 직접 전송 | talkmore.ai 연결 필요 |
...
2. 자체 API 키를 입력
3. 끝 — talkmore.ai에 대한 계정이나 인터넷 연결이 필요하지 않습니다

선택적 클라우드 기능을 자체 백엔드로 연결하려면 빌드 전에 다음 환경 변수를 설정하세요:

| 변수 | 기본값 | 설명 |
|---|---|---|
| `VITE_API_BASE_URL` | `https://talkmore.ai` | 프론트엔드 클라우드 API 기본 URL |
| `API_BASE_URL` | `https://talkmore.ai` | Rust 백엔드 클라우드 API 기본 URL |
